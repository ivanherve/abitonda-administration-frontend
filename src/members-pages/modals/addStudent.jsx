...
import { ENDPOINT, getAuthRequest, postAuthRequest, postRequest, usePrevious } from "../../links/links";
...
export default function AddStudent(props) {
    ...
    const addStudent = () => {
        ...
        fetch(ENDPOINT('student/create'), postAuthRequest(data, token))
            .then(r => r.json())
            .then(r => {
                if (r.status) //console.log(r);
                    swal('Nouvel élève!', `${r.response.Firstname} ${r.response.Lastname} a bien été ajouté`, 'success').then(() => {
                        props.hide();
                    })
    ...
    const prevInfo = usePrevious({ classes });
    useEffect(() => {
        //if (prevInfo.classes !== classes)
