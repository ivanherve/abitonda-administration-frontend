...
import { Button, Card, Col, Form, ListGroup, Nav, Pagination, Row } from "react-bootstrap";
...
export default function Student(props) {
    ...
    useEffect(() => {
        ...
        fetch(ENDPOINT('students/pagination?limit=10&page=' + currentPage), getAuthRequest(token))
            .then(r => r.json())
            .then(r => {
                //console.log(r)
    ...
    return (
        <div>
            <Row>
                ...
                <Col>
                    <Button variant="outline-success" style={{ width: '100%' }} onClick={() => setShowAddStudent(true)}>
            ...
            <Row>
                ...
                <Col>
                    <Card>
                        <Card.Header>
                            <Nav justify variant="tabs" defaultActiveKey="link-0" onSelect={e => e ? setLink(e) : setLink('link-0')}>
                                <Nav.Item>
                    ...
                    <Row>
                        <Col md={{ span: 6, offset: 5 }}>
                            <Pagination>
                                {
                                    pages.map(p =>
                                        <Pagination.Item key={pages.indexOf(p)} onClick={(e) => p.toClick(e)} active={p.idx + 1 === currentPage}>
                                            {p.idx + 1}
                                        </Pagination.Item>
                                    )
                                }
...
function Links(props) {
    if (props.link === 'link-0') {
        ...
        return (
            <GeneralInformation student={props.student} />
        )
    }
